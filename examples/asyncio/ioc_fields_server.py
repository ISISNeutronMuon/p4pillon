...
Uses `IOCMimicServer`, which builds "<name>.<FIELD>" sub-PVs for free for
any base PV in a providers=[] entry -- either an explicit `IOCMimicProvider`
(needed for per-PV overrides, as for EXAMPLE:PV's dtyp_choices/fields below;
passed directly below, same as any other provider -- `IOCMimicServer`
...
will. See `IOCMimicProvider`'s docstring for the full set of tradeoffs
...
from p4pillon.server.records import FIELD_NAMES, IOCMimicProvider, IOCMimicServer
...
async def main():
    ...
    with IOCMimicServer(providers=[base, pvs]):
